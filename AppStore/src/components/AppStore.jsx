...
const AppStore = () => {
    ...
    const filteredApp=
        activeTabId ==='ALL' ? appsList : appsList.filter((eachItem)=>{
            return(
    ...
    const searchInput=(event)=>{
        setsearchedApp(event.target.value)
    }

     const searchBar=filteredApp.filter((eachItem)=>{
        return(
            eachItem.appName.toLocaleLowerCase().includes(searchedApp.toLocaleLowerCase())
        )
    })

    return (
    <div className='h-full items-center w-full  justify-center bg-slate-300 '>
        <h2 className='text-center text-lg font-bold'>App Store</h2>
        <input type="text" placeholder='serch' className='border-1 file:justify-center ' onChange={searchInput}/>
        <div className='flex gap-2 items-center justify-center'>
         {
            tabsList.map((eachItem)=>{
                return(
                    <TabItem isActive={eachItem.tabId=== activeTabId} displayText={eachItem.displayText} tabId={eachItem.tabId} changedTab={changedTab}/>
                )
            })
         }
        </div>
        ...
        {    
            searchBar.map((eachItem)=>{
                return(
                    <AppItem appName={eachItem.appName} imageUrl={eachItem.imageUrl}/>
                )
            })  
